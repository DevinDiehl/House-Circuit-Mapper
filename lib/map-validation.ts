import {z} from 'zod';
import {migrateMap} from './electrical-map';
...
const circuit=z.object({id,name:z.string().max(500),amps:z.number().min(1).max(200),color:z.string().regex(/^#[a-f0-9]{6}$/i)});
...
export function parseMap(value:unknown){const normalized=house.safeParse(value);if(normalized.success)return normalized.data;const legacy=plan.extend({circuits:z.array(circuit).max(200)}).safeParse(value);if(!legacy.success)throw new Error('Invalid electrical map');return house.parse(migrateMap(legacy.data))}

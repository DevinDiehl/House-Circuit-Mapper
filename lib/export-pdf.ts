...
export async function createElectricalPdf(house:HouseMap,loadImage:(floor:Floor)=>Promise<PdfImage|null>,fonts:{regular:Uint8Array;bold:Uint8Array}):Promise<Uint8Array>{
 ...
 for(const floor of house.floors){
  ...
  const used=new Set(floor.nodes.map(n=>n.breaker));const legends=house.circuits.filter(c=>used.has(c.id)).map(c=>({label:`${c.id} - ${c.name} (${c.amps}A)`,color:c.color}));if(floor.nodes.some(n=>!house.circuits.some(c=>c.id===n.breaker)))legends.push({label:'Unassigned',color:unassigned});
